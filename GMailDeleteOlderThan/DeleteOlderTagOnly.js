...
var configDoNotDelete = {
    ...
    deleteBufferDays: 4, // No of days to schedule in advance. You need to set a manual trigger for this.
  };
        ...
        for (var thread = 0; thread < threads.length; thread++) {
          ...
          for (var message = 0; message < messages.length; message++) {
            ...
            if (email.getDate() < cutOff) {
              ...
              // titles.push(
              //     "(" +
              //     email.getDate().toISOString().split("T")[0]
              //      +
              //     ") " +
              //     email.getFrom() +
              //     ": " +
    ...
    run() {
      ...
      for (var filter = 0; filter < this.config.filters.length; filter++) {
        ...
        if (result.deletedCount != 0) {
          ...
          // filterSummary.push("<table border=\"1\" width=\"100%\" style=\"border-color: #000000; border-collapse: collapse;\"><tr><td>Date</td><td>Star</td><td>From</td><td>Subject</td></tr>");
          ...
          // filterSummary.push("</table><br/>&nbsp;<br/>");
          // for (var i = 0; i < result.messageFields.length; i++) {
          //   var isStarred = ""
          //   if (result.messageFields[i].starred) {
          //     isStarred = " ⭐";
          ...
          //     ") " +
          //     result.messageFields[i].from +
          //     ": <a href=\"" + result.messageFields[i].url + "\" target=\"_blank\">" +
          //     result.messageFields[i].subject +
          //     "</a>"
          //   )
          // }
      ...
      if (
        this.config.sendEmails == True &&
        deletedCount > 0
      ) {
        console.log(summary);
        MailApp.sendEmail(subject, summary, Config.emailAddress, "Gmail-Delete-Older-Than");
      }
    }
  }
